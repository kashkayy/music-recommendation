...
export default function MarkerModal({lat, lng, onClose, onSave, userLat, userLng, favorites}){
            ...
            <div className="modal-content">
              {songs.map(({song}, index) => (
                  <div className="song-info" key={index}>
                    ...
                    <div className="favorite-action" title="add to favorites">{isSongFavorited(song)? <FaCheck onClick={() => alert("Song already exists in favorites!")}/> : <IoAddCircleOutline onClick={()=> onSave(song, userLat, userLng)}/>}</div>
                  </div>
            ))}
            </div>
          </div>
