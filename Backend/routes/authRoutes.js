...
import {checkRefreshToken, checkStatus, createUser, login} from '../controllers.js'
...
router.post('/login', async (req,res) => {
  ...
  try{
    ...
    if(!user){
      ...
      const refreshToken = jwt. sign({id: user.id, role: user.role}, process.env.JWT_SECRET_REFRESH_KEY)
...
router.get('/refresh', async (req,res) => {
  ...
  try{
    ...
    jwt.verify(refreshToken, process.env.JWT_SECRET_REFRESH_KEY, (error, user) => {
      if(error) return res.status(403).json({message: "Forbidden", ok: false})
      req.user = user
      })
    const userId = user.id
    const authorized = await checkRefreshToken(userId, refreshToken)
    if (!authorized){
      return res.status(401).json({message: "Refresh token does not exist", ok: false})
    }
    const accessToken = jwt.sign({id: user.id, role: user.role}, process.env.JWT_SECRET_KEY, {expiresIn: '1h'})
    res.status(201).json({ message: "Success", token: accessToken, ok: true})
  }catch(err){
    res.status(500).json({message: "Cannot generate refresh token at the time", ok: false})
  }
})
