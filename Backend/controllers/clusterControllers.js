...
import { getBucketSizeFromZoom, getCoordBounds, roundedCalculator } from '../utils/ZoomHelper.js'
export async function getClustersData({latMin, latMax, lngMin, lngMax, zoom}) {
...
export async function getTopSongs({lat, lng, zoom}) {
    ...
    if(!bucketSize || !roundedLat || !roundedLng) throw new Error("Something went wrong")
    ...
    try {
        ...
        return Object.values(songMap).sort((a, b) => b.count - a.count).slice(0, 30)
    } catch (err) {
